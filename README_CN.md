...
| **你的环境** | **推荐组合** | **核心闭环逻辑** |
|---|---|---|
| 端侧 ≤8GB / 树莓派 / 免费云容器 | **M1 + M6** | 纯规则引擎 + 失败日志闭环 + 看门狗自愈（零本地模型） |
| 端侧 16~32GB / 轻量显卡 | **M1 + M2 + M3 + M5 + M6**（+M4 可选） | **关键链路**: M2 RAG 提供证据 $\to$ M5 小模型低成本打标 $\to$ M3 沉淀 SOP $\to$ M6 守护自愈 |
| 端侧 ≥64GB / 工作站 | **M1-M6 全选** | 完整本地全栈自进化闭环 |
| 生产级 / 云端 | 待 EVO-Pro 发布 | 包含参数内化 (M7) 与 CI 门禁 |
| 多用户 / 团队 | 待 EVO-Team 发布 | 包含跨 Agent 经验共享 (M8) 与权限隔离 |
